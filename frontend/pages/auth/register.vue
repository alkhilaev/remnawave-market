...
const schema = toTypedSchema(
  z
    .object({
      ...
      password: z.string({ message: 'Введите пароль' }).min(6, 'Минимум 6 символов'),
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    toast.error(error.data?.message || 'Ошибка при регистрации');
  } finally {
    loading.value = false;
  }
});
  ...
  <div class="flex flex-col gap-6">
    ...
    <Card>
      <CardHeader class="pb-4">
        <CardTitle class="text-lg"> Регистрация </CardTitle>
        <CardDescription> Создайте аккаунт для покупки VPN-подписок </CardDescription>
      </CardHeader>
      ...
      <CardContent>
        <form class="flex flex-col gap-4" @submit="onSubmit">
          ...
          <Button type="submit" class="w-full" :disabled="loading">
            ...
            <template v-else> Создать аккаунт </template>
          </Button>
        </form>
        ...
        <div class="relative my-4">
          <Separator />
          <span
            class="absolute top-1/2 left-1/2 -translate-1/2 bg-card px-2 text-xs text-muted-foreground"
          >
            или
          </span>
        </div>
